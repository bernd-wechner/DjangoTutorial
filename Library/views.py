...
from .celery import debug_task, debug_task2 
...
def StartOrGetProgress(request):
    ...
    if task_id:
        ...
        print(f"Fetching tasks result")
        r =  AsyncResult(task_id)
        print(f"Got result: {r.info}")
        ...
        #
        # "PROGRESS" - The task is running (this is a custom state, not a Celery built in state)
        #
        ...
        print("About to start debug task")
        async_result = debug_task2.apply_async()
        print("Kick started the debug task")
    ...
    if (state == "STARTED" or state == "PROGRESS"):        
        ...
        response = {'id': task_id, 'result': result, 'complete': True, 'success': True}
